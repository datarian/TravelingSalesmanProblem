...
class TestTspHeuristic(unittest.TestCase):
    ...
    def test_select_available(self):
        t = TravelingSalesPersonProblem(nodes = pd.read_csv("./TSP_411.txt", sep=r'\s+', names=['node', 'x', 'y'], header=None).iloc[130:149,:])
    ...
    def test_shortest_edge(self):
        ...
        t = TravelingSalesPersonProblem(nodes)
        se = ShortestEdge(t)
        se.calculate_cycle()
        print(sum(se.cycle))
        print(se.cycle)
...
class TestImprovementHeuristics(unittest.TestCase):
    ...
    def test_init_ch(self):
        t = TravelingSalesPersonProblem(nodes = pd.read_csv("./TSP_411.txt", sep=r'\s+', names=['node', 'x', 'y'], header=None).iloc[130:169,:])
    ...
    def test_get_predecessor_successor(self):
        t = TravelingSalesPersonProblem(nodes = pd.read_csv("./TSP_411.txt", sep=r'\s+', names=['node', 'x', 'y'], header=None).iloc[140:149,:])
        ch_swap = GreedyLocalSearch(t, Swap)
    ...
    def test_calculated_tour_decrease_loss(self):
        t = TravelingSalesPersonProblem(nodes = pd.read_csv("./TSP_411.txt", sep=r'\s+', names=['node', 'x', 'y'], header=None).iloc[134:145,:])
        ch_swap = GreedyLocalSearch(t, Swap)
        ch_swap.calculate_cycle(save_steps=True)
        print(ch_swap.steps)
